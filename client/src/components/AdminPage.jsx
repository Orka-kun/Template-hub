...
function AdminPage() {
  ...
  const handleBlock = async (id) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/users/${id}/block`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleUnblock = async (id) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/users/${id}/unblock`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
      setUsers(users.map(u => (u.id === id ? { ...u, status: 'active' } : u)));
    } catch (err) {
      console.error('Unblock user error:', err.response?.data || err.message);
    }
  };
  ...
  const handleToggleAdmin = async (id, is_admin) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/users/${id}/admin`,
        { is_admin },
        {
          headers: {
