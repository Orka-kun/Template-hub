  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <span className="font-medium" style={{ color: `var(--card-text)` }}>{title} - {t(`question_types.${type}`)}</span>
      )}
      {isCreator && (
...
function TemplatePage() {
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-6 min-h-screen flex items-center justify-center" style={{ backgroundColor: `var(--div-bg-color)` }}>
        <p className="text-lg animate-pulse" style={{ color: `var(--text-color)` }}>{t('loading')}</p>
      </div>
    );
  }
  ...
  if (!template) {
    return (
      <div className="container mx-auto p-6 min-h-screen flex items-center justify-center" style={{ backgroundColor: `var(--div-bg-color)` }}>
        <p className="text-lg" style={{ color: `var(--text-color)` }}>{t('template.not_found')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-6 min-h-screen" style={{ backgroundColor: `var(--div-bg-color)` }}>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
        <h1 className="text-3xl md:text-4xl font-extrabold  bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 animate-fade-in"
        style={{
        ...
        {isCreator && (
          <button
            ...
            className="mt-4 sm:mt-0 px-5 py-2 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
            style={{
              backgroundColor: isDeletingTemplate ? '#f87171' : '#ef4444',
              color: '#ffffff',
            }}
      ...
      <div className="flex flex-wrap gap-2 sm:gap-4 border-b mb-6" style={{ borderColor: `var(--border-color)` }}>
        {isCreator
      ...
      {tab === 'settings' && isCreator && (
        <div className="p-6 rounded-xl shadow-lg space-y-6 max-w-2xl mx-auto" style={{ backgroundColor: `var(--card-bg)` }}>
          <input
            value={template.title}
            onChange={e => setTemplate({ ...template, title: e.target.value })}
            placeholder={t('template.title')}
            className="w-full p-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm transition-all duration-300"
            style={{
              backgroundColor: `var(--bg-color)`,
              color: `var(--text-color)`,
              borderColor: `var(--border-color)`,
          ...
          <label className="flex items-center space-x-3">
            ...
            />
            <span className="font-medium" style={{ color: `var(--card-text)` }}>{t('template.public')}</span>
          </label>
          <div className="space-y-4">
            ...
            <button
              onClick={handleShare}
              className="px-6 py-3 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
              style={{
                backgroundColor: isSharing ? '#2dd4bf' : '#3E3232',
                color: '#ffffff',
              }}
              onMouseOver={(e) => !isSharing && (e.currentTarget.style.backgroundColor = '#503C3C')}
              onMouseOut={(e) => !isSharing && (e.currentTarget.style.backgroundColor = '#3E3232')}
              disabled={isSharing}
            ...
            { (template.access.length > 0 && (
              <div>
                <p className="font-semibold" style={{ color: `var(--card-text)` }}>{t('template.shared_with')}</p>
                <ul className="mt-2 space-y-2">
                  {template.access.map(a => (
                    <li key={a.user_id} className="rounded-lg p-2" style={{ backgroundColor: `var(--bg-color)`, color: `var(--text-color)` }}>
                      {a.user.email}
                    </li>
                  ))}
                </ul>
          ...
          <button
            ...
            className="px-6 py-3 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
            style={{
              backgroundColor: isSavingSettings ? '#2dd4bf' : '#3E3232',
              color: '#ffffff',
            }}
            onMouseOver={(e) => !isSavingSettings && (e.currentTarget.style.backgroundColor = '#503C3C')}
            onMouseOut={(e) => !isSavingSettings && (e.currentTarget.style.backgroundColor = '#3E3232')}
      ...
      {tab === 'questions' && isCreator && (
        <div className="space-y-6 max-w-2xl mx-auto">
          <div className="p-6 rounded-xl shadow-lg space-y-4" style={{ backgroundColor: `var(--card-bg)` }}>
            <input
              value={questionTitle}
            ...
            <button
              ...
              className="px-6 py-3 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
              style={{
                backgroundColor: isAddingQuestion ? '#2dd4bf' : `var(--table-header-from)`,
                color: '#ffffff',
              }}
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            ...
            >
              {template.questions.length === 0 ? (
                <p className="text-center" style={{ color: `var(--text-color)` }}>{t('template.no_questions')}</p>
              ) : (
      ...
      {tab === 'submit' && !isCreator && (
        <div className="space-y-6 max-w-2xl mx-auto">
          <div className="p-6 rounded-xl shadow-lg space-y-6" style={{ backgroundColor: `var(--card-bg)` }}>
            <h2 className="text-2xl font-semibold border-b-2 border-blue-500 pb-2" style={{ color: `var(--card-text)` }}>{t('template.submit_form_title')}</h2>
            {template.questions
              .filter(q => q.type !== 'fixed_user' && q.type !== 'fixed_date')
              .map(q => (
                <div key={q.id} className="space-y-3">
                  <label className="block font-semibold" style={{ color: `var(--card-text)` }}>
                  ...
                  {(q.type === 'single_line' || q.type === 'multi_line') && (
                    <input
                      ...
                      className={`w-full p-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm transition-all duration-300 ${
                        formErrors[q.id] ? 'border-red-500' : ''
                      }`}
                      style={{
                        backgroundColor: `var(--bg-color)`,
                        color: `var(--text-color)`,
                        borderColor: formErrors[q.id] ? '#ef4444' : `var(--border-color)`,
                      }}
            ...
            <button
              ...
              className="px-6 py-3 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
              style={{
                backgroundColor: isSubmittingForm ? '#2dd4bf' : '#3E3232',
                color: '#ffffff',
              }}
              onMouseOver={(e) => !isSubmittingForm && (e.currentTarget.style.backgroundColor = '#503C3C')}
              onMouseOut={(e) => !isSubmittingForm && (e.currentTarget.style.backgroundColor = '#3E3232')}
      ...
      {tab === 'comments' && (
        <div className="space-y-6 max-w-2xl mx-auto">
          {!isCreator && (
            <div className="p-6 rounded-xl shadow-lg space-y-4" style={{ backgroundColor: `var(--card-bg)` }}>
              ...
              <button
                ...
                className="px-6 py-3 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
                style={{
                  backgroundColor: isAddingComment ? '#2dd4bf' : `var(--table-header-from)`,
                  color: '#ffffff',
                }}
          ...
          {isCreator && (
            <p className="text-center" style={{ color: `var(--text-color)` }}>{t('template.creator_cannot_comment')}</p>
          )}
          {template.comments.length === 0 ? (
            <p className="text-center" style={{ color: `var(--text-color)` }}>{t('template.no_comments')}</p>
          ) : (
            template.comments.map(c => (
              <div
                key={c.id}
                className="p-5 rounded-xl shadow-md border hover:shadow-lg transition-shadow duration-300"
                style={{ backgroundColor: `var(--card-bg)`, borderColor: `var(--border-color)` }}
              >
                <p style={{ color: `var(--card-text)` }}>{c.content}</p>
      ...
      {tab === 'aggregation' && isCreator && (
        <div className="space-y-6 max-w-2xl mx-auto">
          <div className="p-6 rounded-xl shadow-lg space-y-4" style={{ backgroundColor: `var(--card-bg)` }}>
            <h2 className="text-2xl font-semibold border-b-2 border-blue-500 pb-2" style={{ color: `var(--card-text)` }}>{t('template.form_analytics')}</h2>
            <p className="font-medium" style={{ color: `var(--card-text)` }}>{t('template.total_submissions')}: {template.forms?.length || 0}</p>
            <button
              onClick={handleExportCSV}
              className="px-6 py-3 rounded-full transition-colors shadow-lg transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:transform-none"
              style={{
                backgroundColor: isExportingCSV ? '#2dd4bf' : '#3E3232',
                color: '#ffffff',
              }}
              onMouseOver={(e) => !isExportingCSV && (e.currentTarget.style.backgroundColor = '#503C3C')}
              onMouseOut={(e) => !isExportingCSV && (e.currentTarget.style.backgroundColor = '#3E3232')}
          ...
          <div>
            <h3 className="text-xl font-semibold mb-4 border-b-2 border-purple-500 pb-2" style={{ color: `var(--card-text)` }}>
              {t('template.previous_submissions')}
            </h3>
            {!template.forms || template.forms.length === 0 ? (
              <p className="text-center" style={{ color: `var(--text-color)` }}>{t('template.no_submissions')}</p>
            ) : (
              template.forms.map(form => (
                <div
                  key={form.id}
                  className="p-5 rounded-xl shadow-md mb-4 border hover:shadow-lg transition-shadow duration-300"
                  style={{ backgroundColor: `var(--card-bg)`, borderColor: `var(--border-color)` }}
                >
                  <p className="font-medium" style={{ color: `var(--card-text)` }}>{t('template.submitted_by')}: {form.user?.name || t('home.unknown_author')}</p>
                  <p style={{ color: `var(--text-color)` }}>
                    {t('template.submitted_at')}: {new Date(form.createdAt).toLocaleString()}
                  </p>
                  <ul className="mt-3 space-y-2">
                    {form.answers.map(a => (
                      <li key={a.id} className="rounded-lg p-2" style={{ backgroundColor: `var(--bg-color)`, color: `var(--text-color)` }}>
              ...
              template.questions.map(q => {
                ...
                return (
                  <div
                    ...
                    style={{ backgroundColor: `var(--card-bg)`, borderColor: `var(--border-color)` }}
                  >
                    <p className="font-semibold" style={{ color: `var(--card-text)` }}>{q.title}</p>
                    {(q.type === 'single_line' || q.type === 'multi_line' || q.type === 'positive_integer') ? (
                      <ul className="mt-3 space-y-2">
                        {template.forms
                          ...
                          .filter(v => v)
                          .map((value, idx) => (
                            <li key={idx} className="text-sm rounded-lg p-2" style={{ backgroundColor: `var(--bg-color)`, color: `var(--text-color)` }}>
                              {value}
                            </li>
                          )) || <li className="text-sm" style={{ color: `var(--text-color)` }}>{t('template.no_answers')}</li>}
                      </ul>
                    ) : q.type === 'checkbox' ? (
                      ...
                      <p className="mt-3" style={{ color: `var(--text-color)` }}>{t('template.no_data')}</p>
                    )}
